...
void test0() {
  {
    const char* src = "UUVVXX{{ 2*2+3/4%5 }}XX";
...
void test_for() {
  ...
  {
    /* FOR LOOP */
    const char* src = "UUXX{% for key,value in my_dictionary if key == 2+3*4/5-a %}\n" \
  ...
  {
    /* FOR LOOP */
    const char* src = "UVWXYZ{% for _,a in my_dict+(-3) if a != not True %}\n" \
                       "<dt>{{ KeyValue | filter1 }}</dt>\n" \
                       "<dt>{{ a | lower_case }}</dt>\n" \
                       "<dt>{{ 1+2*3%5 + a + 3*2 }}</dt>\n" \
                       "{% endfor %}";
